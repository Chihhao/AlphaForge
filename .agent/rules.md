...
## 🛡️ Token 消耗與效能優化 (Efficiency & Token Saving)

為了降低 Token 消耗並提高開發效率，AI 應遵循以下行為準則：

1. **優先進行程式碼靜態分析**：
   - 當需要理解後端 API 介面、資料結構或邏輯時，應**直接讀取原始碼**（如 `backend/app/api/`、`models/`、`schemas/`），嚴禁頻繁啟動瀏覽器查看 `localhost:8000/docs`。
   - 程式碼是最終的真相來源。
2. **精確的上下文管理**：
   - 避免在大對話中處理過多不相關的任務。
   - 只在必要時讀取大型檔案，並利用 `grep_search` 定位特定邏輯。
3. **減少重複確認**：
   - 一旦確認了專案結構（如路徑設定），應記錄在 Context 中，避免在同一個對話中多次執行 `ls` 或 `list_dir` 遍歷相同目錄。
